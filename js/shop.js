...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const productFiles = ['data/glasses.json', 'data/accessories.json', 'data/care_products.json'];

    // --- Main Initialization Function ---
    function initializeShop() {
        // Fetch all products from JSON files
        Promise.all(productFiles.map(file => fetch(file).then(res => res.json())))
            .then(productArrays => {
                // Use the global product store
                window.siteData.products = productArrays.flat();
                console.log("All products loaded into global store:", window.siteData.products);
            ...
            .catch(error => {
                ...
                if (productGrid) productGrid.innerHTML = '<p>Ошибка загрузки товаров.</p>';
            });
    }

    // --- UI Population Functions ---
    function populateCategoryFilters() {
        const categories = [...new Set(window.siteData.products.map(p => p.category))];
    ...
    function updateDynamicFilterControls(category) {
        const categoryProducts = window.siteData.products.filter(p => p.category === category);
        
        const subcategories = [...new Set(categoryProducts.map(p => p.subcategory))];
        if (subcategoryFilterContainer) {
            subcategoryFilterContainer.innerHTML = subcategories.map(sub => `
    ...
    function addFilterEventListeners() {
        if (categoryChoiceBtns.length > 0) {
            categoryChoiceBtns.forEach(btn => {
                btn.addEventListener('click', () => handleCategoryChoice(btn.dataset.category));
            });
        }
        ...
        if (priceRange) {
            priceRange.addEventListener('input', e => {
                ...
                if (priceMaxValue) priceMaxValue.textContent = `${currentFilters.maxPrice.toLocaleString('ru-RU')} ₽`;
                applyFilters();
            });
        }
    }
        ...
        if (subcategoryFilterContainer) {
            subcategoryFilterContainer.addEventListener('change', (e) => {
                if (e.target.name === 'subcategory') {
                    currentFilters.subcategories = Array.from(document.querySelectorAll('input[name="subcategory"]:checked')).map(el => el.value);
                    applyFilters();
                }
            });
        }
    ...
    function applyFilters() {
        const filteredProducts = window.siteData.products.filter(p => {
            const categoryMatch = currentFilters.category ? p.category === currentFilters.category : true;
            const subcategoryMatch = currentFilters.subcategories.length > 0 ? currentFilters.subcategories.includes(p.subcategory) : true;
            const brandMatch = currentFilters.brands.length > 0 ? currentFilters.brands.includes(p.brand) : true;
            const priceMatch = p.price <= currentFilters.maxPrice;
            const searchMatch = currentFilters.searchTerm ? (p.name.toLowerCase().includes(currentFilters.searchTerm) || p.brand.toLowerCase().includes(currentFilters.searchTerm)) : true;
            
            return categoryMatch && subcategoryMatch && brandMatch && priceMatch && searchMatch;
        });
